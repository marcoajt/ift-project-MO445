...
//  int hierch_levels=(int)iftGetDblValFromDict("hierarch_levels_arg",params);
  int hierch_levels=3;
  ...
  for (int i=0;i<file_count;i++) {
    ...
    t1 = iftTic();
    int ngroups= iftDivideAndConquerRandomlyUnsupOPF(Z, part_size, iftNormalizedCut, kmax1_arg, kmax2_arg,
  hierch_levels);
    t2 = iftToc();
...
int main(int argc, char *argv[])
{
  ...
  iftInsertIntoDict("part_size_arg", iftRange(500, 5000, 1000),params);
  iftInsertIntoDict("kmax1_arg", iftRange(0.005, 0.05, 0.01),params);
  iftInsertIntoDict("kmax2_arg", iftRange(0.005, 0.05, 0.01),params);
//  iftInsertIntoDict("hierarch_levels_arg",iftRange(2,3,1),params);
