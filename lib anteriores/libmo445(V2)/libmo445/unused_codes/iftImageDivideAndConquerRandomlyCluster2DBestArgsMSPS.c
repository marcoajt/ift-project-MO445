...
float find_best_args(void* my_problem, float* theta) {
  ...
  if (theta[0]<500 || theta[0] > 5000 || theta[1] <0.001 || theta[1] > 0.1 || theta[2] <0.001 || theta[2] > 0.1 || theta[3] < 0 || theta[3]>400)
  ...
  for (int i=0;i<file_count;i++) {
    ...
    t1 = iftTic();
    iftDivideAndConquerRandomlyUnsupOPF(Z, part_size, iftNormalizedCut, kmax1_arg, kmax2_arg,hierarch_levels);
    t2 = iftToc();
...
int main(int argc, char *argv[])
{
  ...
  if (argc != 6)
      iftExit("Usage: iftImageDivideAndConquerRandomlyCluster2DBestArgs <dir_orig_img> <dir_label_img> <count_train_img> <limit_nb_superp> <limit_secs>",
              "main");
  ...
  iftMatrixElem(msps->delta, 0, 0) = 1000;
  iftMatrixElem(msps->delta, 0, 1) = 250;
...
//  msps->stopcriteria=0.002;
  ...
  printf("best_hierarch_levels_arg -> %d\n",3);
