...
int main(int argc, char *argv[]) 
{
  ...
  if (argc < 8 || argc >9)
      iftExit("Usage: iftImageDivideAndConquerRandomlyCluster2D <image.ppm[pgm,png]> <partition_size> <kmax_perc1 [0,1]> <kmax_perc2 [0,1]> <hierarchy_levels> <area> <do_smoothing> <gt_image(OPTIONAL)>",
              "main");
...
//  iftRandomSeed((unsigned int)time(NULL));
  ...
  fprintf(stdout,"%s\t%d\t%.3f\t%.3f\t%d\t",argv[1],atoi(argv[2]),atof(argv[3]),atof(argv[4]),atoi(argv[5]));
  ...
  t1 = iftTic();
  iftDivideAndConquerRandomlyUnsupOPF2Levels(Z, atoi(argv[2]), iftNormalizedCut, atof(argv[3]),
                                                          atof(argv[4]),
                                                          atoi(argv[5]));
  t2     = iftToc();
