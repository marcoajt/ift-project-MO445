...
iftFColor iftGetColorFeats(const iftMImage *mimg, int voxel_idx, int img_max_range) {
    ...
    // LAB Color Image
    if (mimg->m == 3) {
        cfeats.val[0] = mimg->band[0].val[voxel_idx]; // L
        cfeats.val[1] = mimg->band[1].val[voxel_idx]; // A
        cfeats.val[2] = mimg->band[2].val[voxel_idx]; // B
    }
    // GRAYSCALE IMAGE
    else {
        cfeats.val[0] = mimg->band[0].val[voxel_idx];
    }
...
void iftEnforceLabelConnectivity(iftImage *superpixels, iftSlic *slic) {
    if (superpixels == NULL)
        iftExit("Superpixel Image is NULL", "iftEnforceLabelConnectivity");
...
void iftUpdateCellCenters(iftSlic *slic, const iftImage *superpixels, const iftMImage *mimg) {
    // CHECKERS
    if (slic == NULL)
        iftExit("Slic is NULL", "iftUpdateCellCenters");
    if (slic->n_cells <= 0) {
        printf("n->cells is equal to 0. Failed\n");
        return;
    }
    if (slic->spatial_feats == NULL)
        iftExit("Slic Spatial Feats are NULL", "iftUpdateCellCenters");
    if (slic->color_feats == NULL)
        iftExit("Slic Color Feats are NULL", "iftUpdateCellCenters");
    if (superpixels == NULL)
        iftExit("Superpixel Image is NULL", "iftUpdateCellCenters");
    if (mimg == NULL)
        iftExit("Multi-band Color Image (LAB or Grayscale) is NULL", "iftUpdateCellCenters");
...
iftImage *iftGenerateSuperpixelsBySlic(const iftImage *img, const iftImage *input_grad, int input_n_cells,
                                       double comp, int img_max_range, int *out_n_cells,iftImage *gt_img) {
    // CHECKERS
  if (img == NULL)
      iftExit("Image is NULL", "iftGenerateSuperpixelsBySlic");
  if (!iftIsIsotropic(img)) {
    ...
    if (iftIs3DImage(img))
      ...
      iftExit(msg, "iftGenerateSuperpixelsBySlic");
  }
  ...
  iftImage *superpixels = iftPerformSuperpixelsSlic(slic, mimg);
    ...
    /* here we need to propagate the result*/
    if (gt_img){
        int *new_labels=iftAllocIntArray(slic->n_cells+1);
        for (int c=0;c<slic->n_cells;c++){
            iftPoint point=slic->spatial_feats[c];
            int p = iftGetVoxelIndex(superpixels, iftPointToVoxel(point));
            new_labels[c+1]=gt_img->val[p];
        }

        for (int p=0;p<superpixels->n;p++){
            int current_label=superpixels->val[p];
            superpixels->val[p]=new_labels[current_label];
        }
...
iftImage *iftGenerateSuperpixelsBySlicInBlocks(const iftImage *img, const iftImage *input_grad, int n_blocks,
    ...
    if (n_blocks <= 0)
        iftExit("Number of Blocks %d <= 0", "iftGenerateSuperpixelsBySlicInBlocks", n_blocks);
    if (img == NULL)
        iftExit("Image is NULL", "iftGenerateSuperpixelsBySlicInBlocks");
