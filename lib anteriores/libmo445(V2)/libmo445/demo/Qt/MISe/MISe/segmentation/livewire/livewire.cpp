...
int *OptimumPath(iftImage *img, const iftImage *saliencyImage, int p1, int p2, int *old_path, LivewireOrientation orientation)
{
    iftGQueue *Q=NULL;
    iftImage *cost, *pred;
        ...
        for (int i=1; i < A->n; i++) {
            ...
            if (iftValidVoxel(img,v)){
                ...
                if (orientation == NONE ||
                    ...
                    tmp = cost->val[p] + Cmax;//iftRound(pow(w,1.5));
